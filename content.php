	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<?php if ( is_sticky() && is_home() && ! is_paged() ) { ?>
			<div class="featured-post">
				<?php esc_html_e( 'Featured post', 'silvercord' ); ?>
			</div>
		<?php } ?>
		<header class="entry-header">
			...
			<?php if ( has_post_thumbnail() && !is_search() ) { ?>
				<a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to ', 'silvercord' ) . '%s', the_title_attribute( 'echo=0' ) ) ); ?>">
					<?php 
		...
		<?php }
			...
			<div class="entry-content">
				<?php 
                                 the_excerpt();
                                /*
                                 the_content( wp_kses( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'silvercord' ), array( 'span' => array( 
					'class' => array() ) ) )
					); 
                                 * 
                                 */
